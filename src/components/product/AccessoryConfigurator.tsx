...
export function AccessoryConfigurator({ product, config, buyable }: Props) {
  ...
  return (
    <div className="mt-8">
      ...
      <div className="mt-6 rounded-2xl border border-titanium/[0.08] bg-base-900/50 p-5">
        <p className="font-heading text-sm text-titanium">Configure your system</p>
              ...
              <label
                ...
                className={`flex cursor-pointer items-start gap-3 rounded-xl border p-4 transition-colors ${
                  isOn
                    ...
                    : "border-titanium/[0.07] bg-base-800/40 hover:border-titanium/20"
                }`}
              >
                <input
                  type="checkbox"
                  ...
                  className="mt-0.5 h-4 w-4 shrink-0 accent-[#244b96]"
                />
                <span className="flex-1">
                  <span className="flex items-center justify-between gap-3">
                    <span className="text-sm text-titanium">{a.name}</span>
                    <span className="whitespace-nowrap text-sm text-titanium">
      ...
      <div className="mt-5 flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-titanium/[0.08] bg-base-900/60 p-5">
        <div>
          <p className="text-xs uppercase tracking-[0.2em] text-steel">
            {buyable ? "Your total" : "Configured estimate"}
          </p>
          <p className="font-heading text-2xl text-titanium">
